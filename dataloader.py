...
class YahooImplicitBCELossDataLoader(BaseImplicitBCELossDataLoader):
    def __init__(self, dataset_path, device, has_item_pool_file= False):
        ...
        if self.has_item_pool:
            self.item_pool_path: str = self.dataset_path + '/random.txt'
            with open(self.item_pool_path, 'r') as inp:
                #inp.readline()
                lines: list = inp.readlines()
                print('Begin analyze item pool file')
                pairs, _, _ = analyse_interaction_from_text(lines, has_value=True)
...
class YahooImplicitDataset:
    def __init__(self, dataset_path: str,seed,ratio=0.5):
        self.dataset_path = dataset_path

        self.norm_data_path = self.dataset_path + '/train.csv'
        self.uniform_data_path = self.dataset_path + '/random.txt'

        self.norm_data_df= pd.read_csv(self.norm_data_path)
        self._norm_train_data = self.norm_data_df.values.astype(np.int64)
        np.random.seed(seed)
        np.random.shuffle(self._norm_train_data)

        self.user_num = max(self.norm_data_df['user_id']) + 1
        self.item_num = max(self.norm_data_df['item_id']) + 1


        uniform_data_df = pd.read_csv(self.uniform_data_path, sep=',', header=None, names=['uid', 'iid', 'rating'])
        uniform_data_df['rating'].loc[uniform_data_df['rating'] < 4] = 0
        uniform_data_df['rating'].loc[uniform_data_df['rating'] >= 4] = 1

        uniform_train_val_df, uniform_test_df = train_test_split(uniform_data_df, test_size=0.9, shuffle=True,random_state=seed)
        uniform_train_df, uniform_val_df = train_test_split(uniform_train_val_df, test_size=ratio, shuffle=True,random_state=seed)

        self.uniform_train_df = uniform_train_df.sort_values(['uid','iid'])
        self.uniform_val_df = uniform_val_df.sort_values(['uid','iid'])
        self.uniform_test_df = uniform_test_df.sort_values(['uid','iid'])

        self._uniform_train_data = self.uniform_train_df.values.astype(np.int64)
        self._uniform_val_data = self.uniform_val_df.values.astype(np.int64)
        self._uniform_test_data = self.uniform_test_df.values.astype(np.int64)

        with open(self.norm_data_path, 'r') as inp:
            inp.readline()
            lines: list = inp.readlines()
            ...
            trained_pairs = list(filter(lambda pair: pair[2] > 0, pairs))

            user_interaction: list = analyse_user_interacted_set(trained_pairs)
...
class CoatImplicitDataset:
    def __init__(self, dataset_path: str,seed):
        self.dataset_path = dataset_path

        self.norm_data_path = self.dataset_path + '/user.txt'
        self.uniform_data_path = self.dataset_path + '/random.txt'

        norm_data_df = pd.read_csv(self.norm_data_path, sep=',', header=None, names=['uid', 'iid', 'rating'])
        norm_data_df['rating'].loc[norm_data_df['rating'] < 4] = 0
        norm_data_df['rating'].loc[norm_data_df['rating'] >= 4] = 1
        ...
        self.user_num = max(self.norm_data_df['uid']) + 1
        self.item_num = max(self.norm_data_df['iid']) + 1
        ...
        uniform_train_val_df, uniform_test_df = train_test_split(uniform_data_df, test_size=0.9, shuffle=True,random_state=seed)
        uniform_train_df, uniform_val_df = train_test_split(uniform_train_val_df, test_size=0.5, shuffle=True,random_state=seed)
...
class Interactions(Dataset):
    def __init__(self, uniform_data,trained_user_interaction):
        self.uniform_data = uniform_data
        self.trained_user_interaction = trained_user_interaction

    def __getitem__(self, index):
        row = self.uniform_data[index][0]
        col = self.uniform_data[index][1]
        val = self.uniform_data[index][2]
        return row, col, val
